...
# Changed this to get small data
...
def prepare_data_no_pool():
    ...
    for batch_id in tqdm(np.arange(args.n_batch)):
        result = author_disambiguation_sample(randint(), sel_train_pairs, train_range, args.batch_size)
        jobs.append(result)
    result = author_disambiguation_sample(randint(), sel_valid_pairs, valid_range, args.batch_size)
    jobs.append(result)
    return jobs
...
#         jobs = prepare_data(pool)
        et = time.time()
        logger.info('Data Preparation: %.1fs' % (et - st))
        ...
        with torch.no_grad():
            ...
            st = time.time()
            logger.info(("Epoch: %d (%.1fs)  LR: %.5f Train Loss: %.2f  Valid Loss: %.2f  Valid NDCG: %.4f  Valid MRR: %.4f") % \
...
with torch.no_grad():
    ...
    logger.info('Test NDCG: %.4f' % np.average(test_ndcg))
    test_mrr = mean_reciprocal_rank(test_res)
    logger.info('Test MRR:  %.4f' % np.average(test_mrr))
