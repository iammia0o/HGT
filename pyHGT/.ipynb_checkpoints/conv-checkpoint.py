...
class HGTConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout = 0.2, use_norm = True, use_RTE = True, use_meta_rela=True, use_msg_rela=True, **kwargs):
        ...
        self.att           = None
        self.use_meta_rela = use_meta_rela
        self.use_msg_rela  = use_msg_rela
        ...
        if self.use_meta_rela:
            self.relation_pri   = nn.Parameter(torch.ones(num_relations, self.n_heads))

        self.relation_att   = nn.Parameter(torch.Tensor(num_relations, n_heads, self.d_k, self.d_k))
        if self.use_msg_rela:
            ...
            for target_type in range(self.num_types):
                ...
                for relation_type in range(self.num_relations):
                    ...
                    if self.use_meta_rela:
                        ...
                        res_att[idx] = (q_mat * k_mat).sum(dim=-1) / self.sqrt_dk
                    '''
...
class GeneralConv(nn.Module):
    def __init__(self, conv_name, in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm = True, use_RTE = True,use_meta_rela=True, use_msg_rela=True):
        super(GeneralConv, self).__init__()
        self.conv_name = conv_name
        if self.conv_name == 'hgt':

            self.base_conv = HGTConv(in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm, use_RTE, use_meta_rela, use_msg_rela)
